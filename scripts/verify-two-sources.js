...
const baseline = JSON.parse(fs.readFileSync('docs/neural/two-source-baseline.json'));
...
try {
  ...
  for (const trial of JSON.parse(fs.readFileSync('docs/neural/two-source-trials.json'))) {
    ...
    report.cases.push({ trial, initial, actions, summary, trace, state });
  ...
  for (const trial of JSON.parse(fs.readFileSync('docs/neural/approach-trials.json'))) {
    ...
    const state = await page.evaluate(() => window.archonDebug.brain.steps(300));
  ...
  fs.writeFileSync('docs/neural/two-source-verification.json', JSON.stringify(report));
  fs.writeFileSync('docs/neural/two-source-summary.json', JSON.stringify({ ...report, cases: report.cases.map(c => ({ ...c.summary, actions: c.actions.map(a => ({ action: a.action, value: a.value, before: a.before.currentCommand, after: a.after.currentCommand })) })) }, null, 2));
